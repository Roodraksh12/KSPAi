    ...
    <div className="bg-surface flex h-[calc(100vh-13rem)] min-h-[560px] flex-col rounded-3xl p-5 min-w-0 border border-hairline shadow-sm">
      <div className="flex items-center justify-between px-1">
        <div>
          ...
          <p className="mt-0.5 text-lg font-semibold tracking-tight text-foreground">{t("ledger.assignedToYou")}</p>
        </div>
        <Badge tone="muted">{cases.length} {t("ledger.casesCount")}</Badge>
      </div>
...
export function CaseCard({ c, compact }: { c: any; compact?: boolean }) {
  ...
  // Status mapping
  let statusKey = `dossier.status.${c.status}`;
  let statusText = t(statusKey) !== statusKey ? t(statusKey) : (c.status ? String(c.status).replace("_", " ") : "");
  ...
  const dateStr =
    ...
    (c.reportedDate
      ...
      : "");
  const stationName = typeof c.station === "string" ? c.station : c.station?.name || t("ledger.yourStation");
